...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD get_file_path.
    ...
    IF sy-subrc <> 0.
      show_message( iv_text = |File dialog error (rc={ sy-subrc })|
                   iv_type = 'W' ).
      RETURN.
    ENDIF.
    ...
    IF lv_rc = 1.
      LOOP AT lt_files ASSIGNING FIELD-SYMBOL(<fs_file>).
        rv_path = <fs_file>-filename.
        EXIT.  "Only first entry needed (multiselection = false)
      ENDLOOP.
    ENDIF.
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD show_results.
    ...
    TRY.
        ...
        lo_sorts->add_sort( columnname = 'TYPE'
